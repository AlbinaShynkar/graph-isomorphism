...
class Graph(object):
    ...
    def __add__(self, other: "Graph") -> "Graph" :
        ...
        dictionary = {}
        disjoint_union = Graph(self.directed or other.directed, 0, self.simple and other.simple)

        for vertex in self.vertices :
            new_vertex = Vertex(disjoint_union)
            dictionary [ vertex ] = new_vertex
            disjoint_union.add_vertex(new_vertex)
        ...
        for e in self.edges + other.edges :
            new_edge = Edge(dictionary [ e.tail ], dictionary [ e.head ])
            disjoint_union.add_edge(new_edge)
